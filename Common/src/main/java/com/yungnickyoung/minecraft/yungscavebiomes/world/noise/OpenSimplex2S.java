...
public class OpenSimplex2S {
    ...
    private static final int GRAD_HASH_MASK_4D = (N_GRADS_4D - 1) << 2;
    ...
    public static void vec3Noise3_ImproveXZ(long seed, double x, double y, double z, Vector3f output) {
        ...
        // Evaluate both lattices to form a BCC lattice.
        vec3Noise3_UnrotatedBase(seed, xr, yr, zr, output);
    }
    ...
    /*
     * Gradient-output 4D noise
     */

    /**
     * 4D SuperSimplex noise with partial derivative vector output,
     * with XYZ oriented like noise3_ImproveXZ,
     * and W for an extra degree of freedom.
     * Recommended for time-varied animations of 3D noise
     * in a space where Y is vertical (W=time)
     */
    public static double noise4Grad_ImproveXYZ_ImproveXZ(long seed, double x, double y, double z, double w, Vector4f gradient) {
        ...
        double value = noise4Grad_UnskewedBase(seed, xr, yr, zr, wr, gradient);

        float xzd = gradient.x() + gradient.z();
        float xyzd = xzd + gradient.y();
        float s2d = xzd * -0.21132486540518699998f;
        float yyd = gradient.y() * 0.57735026918962599998f;
        gradient.set(
            gradient.x() + (s2d - yyd),
            xyzd * 0.28867513459481294226f + gradient.w() * -0.866025403784439f,
            gradient.z() + (s2d - yyd),
            (xyzd + gradient.w()) * 0.5f
        );
    ...
    /**
     * 4D SuperSimplex noise base, with partial derivative vector output.
    ...
    private static float noise4Grad_UnskewedBase(long seed, double xs, double ys, double zs, double ws, Vector4f gradient) {
        ...
        // Point contributions
        float value = 0;
        float dVdx = 0, dVdy = 0, dVdz = 0, dVdw = 0;
        ...
        for (int i = secondaryIndexStart; i < secondaryIndexStop; i++) {
            ...
            if (na < RSQUARED_4D) {
                na -= RSQUARED_4D;
                float aa = na * na, naaa = aa * na, aaaa = aa * aa;

                int gi = v4GradIndex(seed, xsvp + c.xsvp, ysvp + c.ysvp, zsvp + c.zsvp, wsvp + c.wsvp);
                float gx = GRADIENTS_4D[gi | 0], gy = GRADIENTS_4D[gi | 1], gz = GRADIENTS_4D[gi | 2], gw = GRADIENTS_4D[gi | 3];
                float rampValue = gx * dx + gy * dy + gz * dz + gw * dw;

                value += aa * aa * rampValue;
                dVdx += gx * aaaa + 8 * rampValue * naaa * dx;
                dVdy += gy * aaaa + 8 * rampValue * naaa * dy;
                dVdz += gz * aaaa + 8 * rampValue * naaa * dz;
                dVdw += gw * aaaa + 8 * rampValue * naaa * dw;
            }
        }
    ...
    private static int v4GradIndex(long seed, long xsvp, long ysvp, long zsvp, long wsvp) {
        ...
        return (int)hash & GRAD_HASH_MASK_4D;
    }
    ...
    // Adding/rejecting points in regular tetrahedral quartets at a time ensures that the final set averages out to zero.
    // The algorithm was re-run with adjusted minimum point spacing until a result with exactly 256 vectors was returned.
